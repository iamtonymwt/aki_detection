#!/usr/bin/env python3
import warnings
from handler.listener import start_listener, receive_message, ack_message, close_connection
from handler.hl7_processor import parse_hl7_message, extract_mrn
from handler.data_processor import load_and_process_history, get_patient_history, update_patient_data
from handler.aki_detector import load_model, aggregate_data, predict_aki
from handler.pager import send_pager_message
from utils.f3_evaluation import check_aki_detection_accuracy
import argparse
import pandas as pd
import os
import signal
...
def main():
    ...
    if not reload_csv_from_shutdown():
        ...
        if args.local:
            historical_data = load_and_process_history('/model/data/history.csv')
        else:
